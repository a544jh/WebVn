...
type Prompt = AnimatablePrompt PromptParams
...
type alias ToLabel = String

type Command = SpriteCmd | BgCmd

type TextCommand = Command | AppendText TextNode
...
type alias NameTag = Maybe TextNode
...
advance player =
  let
    nextIndex = player.promptIndex + 1
    nextPrompt = List.take nextIndex player.story |> List.head
  in
    case nextPrompt of
      Just prompt ->
        case prompt of
